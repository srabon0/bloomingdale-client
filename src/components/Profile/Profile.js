...
const Profile = () => {
  ...
  if (!user){
	...
	<div className="flex-shrink-0 w-full mb-6 h-44 sm:h-32 sm:w-32 sm:mb-0">
		<img src={user?.photoURL || "https://source.unsplash.com/100x100/?portrait?1"} alt="" className="object-cover object-center w-full h-full rounded dark:bg-coolGray-500"/>
	</div>
	<div className="flex flex-col space-y-4">
		<div>
			<h2 className="text-2xl font-semibold">{user?.email.split("@")[0]}</h2>
			<span className="text-sm dark:text-coolGray-400">General manager</span>
		</div>
